...
function showError($app, err) {
    ...
    const bodyEn = "Please refresh. If it keeps happening, the register's display files may not be published on the server yet — open the Classic register below, and ask your administrator to rebuild assets (bench build).";
    const bodyAr = "يرجى التحديث. إذا استمرّت المشكلة، فقد لا تكون ملفات العرض منشورة على الخادم — افتح كاشير Classic بالأسفل، واطلب من المسؤول إعادة بناء الملفات.";
    ...
    // cashier can read this card and use the Classic register instead).
    try {
