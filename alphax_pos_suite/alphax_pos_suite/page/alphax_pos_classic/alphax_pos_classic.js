frappe.pages['alphax-pos-classic'].on_page_load = function(wrapper) {
  ...
  $root.html(`
    <div class="alphax-pos-shell">
      ...
      <!-- Accessibility status (announced to screen readers) + controls -->
      <div class="alphax-a11y-bar">
        <div class="alphax-a11y-status" role="status" aria-live="assertive" aria-atomic="true" data-area="a11y">Ready. Scan an item or type a code, then press Enter.</div>
        <div class="alphax-a11y-controls">
          <button class="alphax-btn alphax-btn-ghost alphax-btn-sm" data-action="toggle_contrast" aria-pressed="false" title="High contrast & large text (Alt+A)">&#9855; Big &amp; Clear</button>
          <button class="alphax-btn alphax-btn-ghost alphax-btn-sm" data-action="toggle_voice" aria-pressed="false" title="Speak actions aloud (Alt+V)">&#128266; Voice</button>
          <button class="alphax-btn alphax-btn-ghost alphax-btn-sm" data-action="show_shortcuts" title="Keyboard shortcuts (Alt+/)">&#9000; Keys</button>
        </div>
      </div>
...
async function load_settings(){
  ...
  // The cashier can run this register without watching the screen: every
  // action is announced to screen readers (aria-live), confirmed with a
  // beep, and optionally spoken aloud.
  let _ac = null;
  function beep(ok){
    try{
      ...
      g.gain.value = 0.05;
      o.start();
      o.stop(_ac.currentTime + (ok ? 0.09 : 0.20));
    }catch(e){}
  }
  let _voice = (localStorage.getItem('alphax_pos_voice') === '1');
  function speak(msg){
    if (!_voice) return;
    try { const u = new SpeechSynthesisUtterance(msg); u.rate = 1.05;
      window.speechSynthesis.cancel(); window.speechSynthesis.speak(u); } catch(e){}
  }
  function cur(){ return state.currency || 'SAR'; }
  function focus_scan(){ try { $(wrapper).find('[data-field="scan"]').focus(); } catch(e){} }
  function announce(msg, opts){
    opts = opts || {};
    $(wrapper).find('[data-area="a11y"]').text(msg);
    if (opts.ok !== undefined) beep(opts.ok);
    speak(msg);
  }
  ...
  async function add_item(){
    ...
    if(!code){ announce('Nothing to add. Scan a barcode or type an item code.', {ok:false}); focus_scan(); return; }
    const rate=await pricing_get(code);
    state.cart.push({item_code:code, qty, rate});
    $(wrapper).find('[data-field="scan"]').val('');
    render_cart();
    const total = state.cart.reduce((t,r)=>t + r.qty*r.rate, 0);
    announce(`Added ${code}, quantity ${qty}, ${(qty*rate).toFixed(2)} ${cur()}. Total ${total.toFixed(2)} ${cur()}.`, {ok:true});
    focus_scan();
  }
  ...
  $(wrapper).on('click','[data-action="inc"]', function(){ const i=parseInt($(this).attr('data-idx')); state.cart[i].qty+=1; render_cart(); announce(`${state.cart[i].item_code} quantity ${state.cart[i].qty}. Total ${ui_total().toFixed(2)} ${cur()}.`, {ok:true}); });
  $(wrapper).on('click','[data-action="dec"]', function(){ const i=parseInt($(this).attr('data-idx')); state.cart[i].qty=Math.max(1,state.cart[i].qty-1); render_cart(); announce(`${state.cart[i].item_code} quantity ${state.cart[i].qty}. Total ${ui_total().toFixed(2)} ${cur()}.`, {ok:true}); });
  $(wrapper).on('click','[data-action="rm"]', function(){ const i=parseInt($(this).attr('data-idx')); const nm=(state.cart[i]||{}).item_code||'item'; state.cart.splice(i,1); render_cart(); announce(`Removed ${nm}. Total ${ui_total().toFixed(2)} ${cur()}.`, {ok:false}); focus_scan(); });
  ...
  // ---- Accessibility toggles + keyboard shortcuts ---------------------
  function set_contrast(on){
    $('body').toggleClass('alphax-pos-a11y-high', !!on);
    $(wrapper).find('[data-action="toggle_contrast"]').attr('aria-pressed', on?'true':'false');
    localStorage.setItem('alphax_pos_a11y', on?'1':'0');
  }
  function set_voice(on){
    _voice = !!on;
    $(wrapper).find('[data-action="toggle_voice"]').attr('aria-pressed', on?'true':'false');
    localStorage.setItem('alphax_pos_voice', on?'1':'0');
    if(on) announce('Voice announcements on.', {ok:true});
    else $(wrapper).find('[data-area="a11y"]').text('Voice announcements off.');
  }
  function show_shortcuts(){
    frappe.msgprint({ title:'Keyboard shortcuts', indicator:'blue', message:`
      <table class="table table-bordered" style="margin:0">
        <tr><td><b>Enter</b></td><td>Add the scanned / typed item</td></tr>
        <tr><td><b>Alt + S</b></td><td>Jump to the Scan box</td></tr>
        <tr><td><b>Alt + P</b></td><td>Pay &amp; Submit</td></tr>
        <tr><td><b>Alt + H</b></td><td>Hold the sale</td></tr>
        <tr><td><b>Alt + R</b></td><td>Return</td></tr>
        <tr><td><b>Alt + K</b></td><td>Pay by Card</td></tr>
        <tr><td><b>Alt + Backspace</b></td><td>Remove the last line</td></tr>
        <tr><td><b>Alt + A</b></td><td>Big &amp; Clear mode (high contrast)</td></tr>
        <tr><td><b>Alt + V</b></td><td>Voice announcements on / off</td></tr>
      </table>` });
  }
  $(wrapper).on('click','[data-action="toggle_contrast"]', ()=> set_contrast(!$('body').hasClass('alphax-pos-a11y-high')));
  $(wrapper).on('click','[data-action="toggle_voice"]', ()=> set_voice(!_voice));
  $(wrapper).on('click','[data-action="show_shortcuts"]', show_shortcuts);

  function on_key(e){
    if (frappe.get_route_str().indexOf('alphax-pos-classic') === -1) return;
    if (!e.altKey || e.ctrlKey || e.metaKey) return;
    const k = (e.key||'').toLowerCase();
    const map = {
      's': focus_scan,
      'p': ()=>create_order({hold:false, submit:true}),
      'h': ()=>create_order({hold:true, submit:false}),
      'r': ()=>create_order({hold:false, submit:true, is_return:true}),
      'k': ()=>pay_by_card(),
      'a': ()=>set_contrast(!$('body').hasClass('alphax-pos-a11y-high')),
      'v': ()=>set_voice(!_voice),
      '/': show_shortcuts,
      'backspace': ()=>{ if(state.cart.length){ const r=state.cart.pop(); render_cart(); announce(`Removed ${r.item_code}. Total ${ui_total().toFixed(2)} ${cur()}.`, {ok:false}); focus_scan(); } },
    };
    if (map[k]){ e.preventDefault(); map[k](); }
  }
  $(document).on('keydown.alphaxpos', on_key);
...
// Restore saved accessibility preferences + put the cursor on the scan box
// so the cashier can start scanning immediately without clicking anything.
// Big & Clear (high-contrast / large-text) is ON by default for clarity.
// It only stays off if a cashier explicitly turned it off on this device.
if (localStorage.getItem('alphax_pos_a11y') !== '0') set_contrast(true);
if (localStorage.getItem('alphax_pos_voice') === '1') set_voice(true);
setTimeout(focus_scan, 250);
...
frappe.router.on('change', function cleanup(){
    if (frappe.get_route_str().indexOf('alphax-pos-classic') === -1) {
        ...
        $(document).off('keydown.alphaxpos');
