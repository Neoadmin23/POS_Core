...
async function reallyClose() {
  error.value = ''; busy.value = true
  try {
    zData.value = await store.closeShift(parseFloat(countedCash.value || 0), closeNotes.value || null)
    mode.value = 'zdone'
  } catch (e) { return fail(e) }
  busy.value = false
}
    ...
    <div class="sh-body">
      ...
      <template v-else-if="mode === 'status' && s">
        <div class="sh-grid" id="alphax-shift-x">
          ...
          <div class="sh-kv"><span>{{ t('shift.net_sales') }}</span><b class="tnum">{{ fmt(s.net_sales) }}</b></div>
      ...
      <template v-else-if="mode === 'zdone' && zData">
        ...
        <div class="sh-grid">
          <div class="sh-kv"><span>{{ t('shift.net_sales') }}</span><b class="tnum">{{ fmt(zData.net_sales) }}</b></div>
        ...
        <div class="sh-actions wrap">
          ...
          <button class="btn" :disabled="busy" @click="doDayClose">{{ t('shift.day_close_btn') }}</button>
          <div class="spacer"></div>
      ...
      <div id="alphax-shift-print" class="sh-print">
        ...
        <template v-if="zData">
          ...
          <div class="p-line"><span>{{ t('shift.net_sales') }}</span><span>{{ fmt(zData.net_sales) }}</span></div>
        ...
        <template v-else-if="s">
          ...
          <div class="p-line"><span>{{ t('shift.net_sales') }}</span><span>{{ fmt(s.net_sales) }}</span></div>
