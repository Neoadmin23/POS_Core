...
const emit = defineEmits(['toggle-rail', 'open-shift', 'hold', 'recall', 'add-customer', 'scan-loyalty', 'open-floor', 'open-held', 'open-hardware', 'open-queue', 'start-return', 'open-history', 'open-config'])
...
<template>
  ...
  >
    ...
    <div class="quick-actions">
      ...
      <div class="qa-divider" v-else></div>
      <button class="qa-btn" :title="t('config.title')" @click="$emit('open-config')">
        <span class="qa-icon">⚙</span>
        <span v-if="!rail">{{ t('config.title') }}</span>
      </button>
    </div>
